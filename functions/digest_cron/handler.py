...
def _get_users_due_now(now_utc: datetime) -> list[dict]:
    ...
    # Exclude cancelled (status=inactive) and seasonally paused users. NE also
    # matches items missing the attribute, so users with neither field set are
    # still included.
    resp = table.scan(
        FilterExpression=Attr("status").ne("inactive") & Attr("paused").ne(True)
    )
...
def _process_user_digest(user: dict, now_utc: datetime):
    ...
    # Defense in depth: the scan filter already excludes paused users, but guard
    # here too so a paused user never receives a digest.
    if user.get("paused"):
        print(f"User {user_id} is paused; skipping digest.")
        return
...
def _self_service_token(user_id: str) -> str:
    """HMAC token for self-service account links (pause/resume/account view).

    Mirrors web/src/lib/tokens.ts: full-length hex over "account:" + user_id,
    signed with the same UNSUBSCRIBE_SECRET so links verify on the web side.
    """
    return hmac.new(
...
def _account_url(user_id: str) -> str:
    token = _self_service_token(user_id)
    return f"{BASE_URL}/account?uid={user_id}&token={token}"
    ...
    <div style="background:#fff;border-radius:0 0 12px 12px;padding:20px 24px;
        ...
        <div style="text-align:center;font-size:12px;color:#aaa;margin-top:20px;
            ...
            <p style="margin:6px 0;">
                ...
                &middot;
                <a href="{account_url}"
                   style="color:#aaa;text-decoration:underline;">Pause or manage</a>
                &middot;
                <a href="{unsub_url}"
