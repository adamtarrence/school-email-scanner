...
def _process_user_digest(user: dict, now_utc: datetime):
    ...
    if not emails:
        print(f"No emails for {user_id}, skipping digest.")
        return
    ...
    html_body = _build_html(digest_md, date_str, user_email)
...
def _build_html(digest_md: str, date_str: str, user_email: str) -> str:
